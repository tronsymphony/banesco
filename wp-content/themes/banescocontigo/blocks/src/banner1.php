        ...
        <div class="container">
            <div class="text-image-slider__inner">
                <div class="text-image-slider__image"><img decoding="async"
                        src="<?= $attributes['items1'][0]['banner_image']['default']['url']; ?>"
                        alt="image"></div>
                <div class="text-image-slider__text">
                    <h2 class="text-image-slider__title"><?= $attributes['items1'][0]['banner_title']; ?></h2>
                    <div class="text-image-slider__content">
                    <?= $attributes['items1'][0]['banner_text']; ?>
                    </div>
                    <?php
                    if(!empty($attributes['items1'][0]['initial_opened'])): ?>
                    <div class="text-image-slider__modal text-image-slider__modal-popup-feature">+ Ver todos los detalles</div>
                    <?php endif; ?>
                </div>
            </div>
        </div>
...
<div class="text-image-slider__modal-content">
    <h2 class="text-image-slider__modal-content-title">Otros beneficios de DANI</h2>
    <p>
        ...
        * La funcionalidad de desembolso está disponible para clientes de Tarjeta de Crédito con MultiCrédito, que
        posean cuenta de ahorro o corriente en Banesco.
    </p>
    <span class="text-image-slider__modal-close">
...
<div class="text-image-slider__modal-content-overlay"></div>

<script>
    const elements = document.getElementsByClassName("text-image-slider__modal-popup-feature");

if (elements.length > 0) {
	document.querySelectorAll('.text-image-slider__modal-popup-feature').forEach(function(item) {
