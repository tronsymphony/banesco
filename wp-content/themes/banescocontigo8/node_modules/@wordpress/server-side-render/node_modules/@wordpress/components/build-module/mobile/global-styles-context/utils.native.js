...
export function getBlockColors(blockStyleAttributes, defaultColors, blockName, baseGlobalStyles) {
  ...
  const blockGlobalStyles = baseGlobalStyles?.blocks?.[blockName]; // Global styles colors.
...
export function getMappedValues(features, palette) {
  ...
  const fontSizes = { ...typography?.fontSizes?.theme,
...
export function useMobileGlobalStylesColors(type = 'colors') {
  ...
  const availableThemeColors = colorGradientSettings?.[type]?.reduce((colors, origin) => colors.concat(origin?.[type]), []); // Default editor colors/gradients if it's not a block-based theme.
